...
# Helps in creating a wide range of static, animated and interactive visualizations.
import matplotlib.pyplot as plt

# Used to import a specialized dictionary that provides a default value for keys that are not present in the dictionary.
...
file_count_level = [0 for i in range(10)]           # Initializing a list to maintain count of files at each recursive level.

link_type_count = defaultdict(int)                      # A default dictionary to count the number of files of each type.
...
def scrape(site, depth, base_url):
    ...
    print(f"At recursion level {args.threshold-depth+1}:")
    print("Crawling:", site)
    print("-" * 30)
    ...
    for i in s.find_all("a"):
        ...
        if 'href' in i.attrs:
            ...
            if link not in visited_urls:
                ...
                file_count_level[depth]+=1          # Increase the file count of that recursive level by one.
                parsed_link = urlparse(link)        # Parse the link and extract its componenets.
    
                # Extract the file extension from the link path or the link type
                file_extension = os.path.splitext(parsed_link.path)[1][1:].lower()
                link_type = None                    # Initializing. Used to store the type of the link based on the file extension or other criteria.
                
                if file_extension:
                    link_type = file_extension
                else:
                    # Additional logic to determine the link type based on other criteria
                    # For example, you can check if the link contains certain keywords
                    if "css" in parsed_link.path:
                        link_type = "CSS"
                    elif "js" in parsed_link.path:
                        link_type = "JavaScript"
                    elif "html" in parsed_link.path:
                        link_type = "HTML"
                    elif "jpg" or "jpeg" or "png" or "gif" or "bmp" in parsed_link.path:
                        link_type = "Image"
                    elif ".pdf" in parsed_link.path:
                        link_type = "Pdf"
                    elif "doc" or "docx" or "txt" in parsed_link.apth:
                        link_type = "Documents"                
                # Increment the count for the link type in the link_type_count dictionary
                if link_type:
                    link_type_count[link_type] += 1
                else:
                    link_type_count["Other"] += 1
    ...
    print("Total files found:", file_count_level[depth])
...
if __name__ == "__main__":
    ...
    link_types = list(link_type_count.keys())               # Creates a list containing the keys of the dictionary. It represents the different link types.
    counts = list(link_type_count.values())                 # Creates a list containing the values of the dictionary. It represents the counts of each link type.

    # Generates x-axis values for the plot (link types)
    x = range(len(link_types))

    # Plot the link type counts
    plt.bar(x, counts)

    # Adding x-axis labels. Labels will be displayed vertically.
    plt.xticks(x, link_types, rotation='vertical')
    ...
    # Saving the plot.
    plt.savefig("Type_of_Files.pdf")
    plt.close()

    x = range(len(file_count_level))           # X-axis values of the plot for recursive levels.
    correct_file_count_level = file_count_level[1:args.threshold+1][::-1]
    # Plot the file count data
    plt.plot(range(1, args.threshold+1), correct_file_count_level)
    plt.xticks(x)
    plt.xlim(1, args.threshold+1)

    # Add labels and title
    plt.xlabel('Recursive Depth Level')
    plt.ylabel('Number of Links')
    plt.title('Number of Links at Each Recursive Level')

    # Display the plot
    # plt.show()

    # Saving the plot.
    plt.savefig("Count_of_files_level.pdf")
    plt.close()
